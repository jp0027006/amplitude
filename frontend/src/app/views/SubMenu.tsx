...
const SubMenu = ({ submenuItems }: SubMenuProps) => (
  <div className="absolute left-0 top-16 mt-2 w-[1535px] bg-white border border-gray-200 p-6 px-20 flex gap-8">
    {submenuItems.map((submenuItem, submenuItemIndex) => (
      <div key={submenuItemIndex} className="flex flex-row gap-10">
        {submenuItem.ImageTitleDescriptionLink && (
          <div
            key={submenuItemIndex}
            className="flex flex-col card rounded-tr-2xl rounded-bl-2xl w-72"
          >
            {submenuItem.ImageTitleDescriptionLink &&
              submenuItem.ImageTitleDescriptionLink.submenuImage && (
                <img
                  src={urlFor(
                    submenuItem.ImageTitleDescriptionLink.submenuImage.asset
                      ?._ref
                      ? {
                          asset: {
                            _ref: submenuItem.ImageTitleDescriptionLink
                              .submenuImage.asset?._ref,
                          },
                        }
                      : undefined
                  )}
                  alt={
                    submenuItem.ImageTitleDescriptionLink.submenuImage.alt ||
                    "Sub Menu image"
                  }
                  className="w-full"
                  width="100"
            ...
            <div className="px-6 py-4 flex flex-col gap-2">
              <h1 className="font-medium text-[22px] leading-[26px]">
                {submenuItem.ImageTitleDescriptionLink.submenuTitle}
              </h1>
              <span className="text-[14px] leading-[20px]">
                {submenuItem.ImageTitleDescriptionLink.submenuDescription}
              </span>
              <Link
                href={
                  submenuItem.ImageTitleDescriptionLink.submenuLink.link
                    ?.link || "/"
                }
                className="mt-4 inline-block text-blue-600 font-semibold text-base"
              >
                {submenuItem.ImageTitleDescriptionLink.submenuLink.label}
              </Link>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-row">
          <div>
            <div className="flex w-full flex-col gap-8 lg:flex-row">
              {submenuItem.LinkDescriptionIcon &&
                Object.entries(
                  submenuItem.LinkDescriptionIcon.reduce((acc, item) => {
                    if (!acc[item.linkCategory]) {
                      acc[item.linkCategory] = [];
                    }
                    acc[item.linkCategory].push(item);
                    return acc;
                  }, {} as Record<string, typeof submenuItem.LinkDescriptionIcon>)
